#############################################
###---Sorting Bowhead .sam file to .bam---###
#############################################
#
...
java -Xmx30G -jar ${GATK_JAR} SortSam \
  -I ~/scratch/temp_mapping/$SAMPLELANE-aln_min1Mb_SRW.sam \
  -O ~/scratch/mapped_reads/$SAMPLELANE-aln-sorted_min1Mb_SRW.bam \
  -SO coordinate \
  --CREATE_INDEX true --MAX_RECORDS_IN_RAM 500000 \
  --TMP_DIR ~/scratch/temp_mapping/ \
  2> QC/std_out/$SAMPLELANE-SortSam_SRW.out
