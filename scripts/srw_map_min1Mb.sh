#########################################
###---   Mapping SRW to reference  ---###
#########################################
#
# This script maps the SRW fastq reads to the
# scaffolds of the Eubalaena australis reference  
...
#	- file 'srw_fastq_list' that specifies samples by lane to be mapped
#	- reference saved to reference/RWref_HiC_min1Mb.fasta
...
SAMPLELANE=$(sed -n "${SLURM_ARRAY_TASK_ID}p" srw_fastq_list)
...
bwa mem -M -t 28 \
  -R $READGROUP \
  reference/RWref_HiC_min1Mb.fasta \
